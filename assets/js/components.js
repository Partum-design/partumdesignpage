(function () {
    ...
    var NAV_CSS = [
        ':root{--dark-bg:#010a13;--accent:#0ea5e9;--nav-height:78px;--nav-height-scrolled:68px;--text-white:#fff;--text-muted:rgba(255,255,255,.72);--glass-bg:rgba(4,14,24,.68)}',
        ...
        '.logo{display:block;width:232px;height:50px;transition:width .34s ease,height .34s ease;flex-shrink:0;z-index:100000;position:relative}',
        '.logo-img{width:100%;height:100%;object-fit:contain;object-position:left center}',
        '.navbar.scrolled .logo{width:204px;height:44px}',
        ...
        '.navbar{top:10px;width:calc(100% - 24px);height:66px;border-radius:16px}',
        '.navbar.scrolled{height:64px}',
        '.nav-container{padding:0 16px}',
        '.logo{width:188px;height:40px}',
        '.mobile-toggle{display:flex}',
        '.nav-content-wrapper{position:fixed;top:76px;left:50%;width:calc(100vw - 24px);height:auto;max-height:calc(100svh - 96px);overflow-y:auto;background:rgba(4,12,22,.96);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,.1);border-radius:18px;box-shadow:0 22px 54px rgba(0,0,0,.42);flex-direction:column;justify-content:flex-start;align-items:center;opacity:0;visibility:hidden;transform:translate(-50%,-12px);transition:opacity .34s,transform .34s,visibility .34s;padding:22px;z-index:99998}',
